package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-cutoff/internal/store")
...
func(s *Server)routes(){
    ...
    s.mux.HandleFunc("POST /api/links/{id}/toggle",s.handleToggle)
    s.mux.HandleFunc("DELETE /api/links/{id}",s.handleDelete)
    s.mux.HandleFunc("GET /r/{code}",s.handleRedirect)
    s.mux.HandleFunc("GET /",s.handleUI)
}
func(s *Server)handleHealth(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-cutoff"})}
