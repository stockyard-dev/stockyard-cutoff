...
func or(r []store.Rule) []store.Rule {
	if r == nil {
		return []store.Rule{}
	}
	return r
...
func (s *Server) listRules(w http.ResponseWriter, r *http.Request) {
	wj(w, 200, map[string]any{"rules": or(s.db.ListRules())})
}
...
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	st := s.db.Stats()
	wj(w, 200, map[string]any{"service": "cutoff", "status": "ok", "rules": st.Rules})
}
...
func (s *Server) loadPersonalConfig() {
	...
	if err := json.Unmarshal(data, &cfg); err != nil {
		log.Printf("%s: warning: could not parse config.json: %v", "cutoff", err)
		return
	}
	s.pCfg = cfg
	log.Printf("%s: loaded personalization from %s", "cutoff", path)
}
