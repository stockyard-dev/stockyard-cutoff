...
import (
	...
	"github.com/stockyard-dev/stockyard-cutoff/internal/license"
	"github.com/stockyard-dev/stockyard-cutoff/internal/server"
	"github.com/stockyard-dev/stockyard-cutoff/internal/store"
)

func main() {
	port := getEnv("PORT", "9330")
	dataDir := getEnv("DATA_DIR", "./data")
	licenseKey := os.Getenv("CUTOFF_LICENSE_KEY")
	...
	log.Printf("Stockyard Cutoff listening on :%s (tier: %s)", port, tier)
